...
async def main():
    ...
    with open('wallets_cfx.txt', 'r') as file:
        wallet_addresses = [line.strip() for line in file if line.strip()]

    # Create a CSV file to store the data
    with open('wallet__cfx_data.csv', 'w', newline='') as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(['Wallet', 'Native Token Amount (CFX)', 'TX Count'])
...
if __name__ == "__main__":
    asyncio.run(main())
    print("Data has been written to wallet_cfx_data.csv")
